...
static int read_cpu_snapshot(CpuRawSnapshot *snap) {
    ...
    /* Parse the eight tick fields from the first "cpu " line */
...
int cpu_get_stats(CpuStats *out_stats) {
    ...
    /* Compute totals for current sample */
    ...
    if (delta_total > 0 && delta_total >= delta_idle) {
        usage = (double)(delta_total - delta_idle) / (double)delta_total * 100.0;
    }
